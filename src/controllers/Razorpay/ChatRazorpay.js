...
export const paymentService = {
  ...
  async createOrder(userId, planId, couponCode = null) {
    try {
      ...
      if (couponCode) {
        const couponResult = await this.processCoupon(
          couponCode,
          userId,
          plan.price,
          plan,
      ...
      throw error instanceof ApiError
        ? error
        : new ApiError(500, `Order creation failed: ${error.message}`);
    }
  },
  ...
  async verifyAndActivate(userId, planId, paymentData, couponCode = null) {
    try {
      ...
      let paymentDetails;
      try {
        paymentDetails = await this.processPayment(
          paymentData,
  ...
  async handleWebhook(req) {
    try {
      ...
      if (handlers[event]) {
        await handlers[event].call(
          this,
          payload.payment?.entity || payload.subscription?.entity,
        );
      } else {
      ...
      throw error instanceof ApiError
        ? error
        : new ApiError(500, `Webhook processing failed: ${error.message}`);
    }
  },
  ...
  async processCoupon(couponCode, userId, amount, planId) {
    try {
      ...
      if (
        coupon.applicablePricingIds.length > 0 &&
      ...
      switch (coupon.discountType) {
        case "percentage":
          ...
          extendedDays =
            ...
            (amount / (amount / plan.validityDays));
  ...
  async createSubscription(userId, planId, paymentDetails) {
    try {
      ...
      expiryDate.setDate(
        expiryDate.getDate() + plan.validityDays + extendedDays,
      );
      ...
      if (paymentDetails.couponCode && paymentDetails.status === "success") {
        const coupon = await Coupon.findOne({
          code: paymentDetails.couponCode,
        });
        if (coupon) {
      ...
      throw error instanceof ApiError
        ? error
        : new ApiError(500, `Failed to create subscription: ${error.message}`);
    }
  },
  ...
  async handlePaymentSuccess(payment) {
    try {
      ...
      if (!order.notes || !order.notes.userId || !order.notes.planId) {
        throw new ApiError(
          400,
          "Missing user or plan information in order notes",
        );
      }
        ...
        const updatedSub = await ChatUserPremium.findOneAndUpdate(
          { "payment.transactionId": payment.order_id },
          {
            $set: {
              ...
              "payment.signature":
                payment.signature || existingSub.payment.signature,
      ...
      const expiryDate = new Date();
      expiryDate.setDate(
        expiryDate.getDate() + plan.validityDays + (extendedDays || 0),
      );
      ...
      throw error instanceof ApiError
        ...
        : new ApiError(
            500,
            `Payment success handling failed: ${error.message}`,
          );
    }
  },
  ...
  async recordFailedPayment(orderId, paymentId, amount, error) {
    try {
      ...
      try {
        ...
        console.error(
          "Fallback failed payment recording also failed:",
          fallbackError,
        );
      }
    }
  ...
  verifyWebhookSignature(req) {
    try {
      ...
      throw error instanceof ApiError
        ? error
        : new ApiError(500, `Webhook verification failed: ${error.message}`);
    }
  },
};
