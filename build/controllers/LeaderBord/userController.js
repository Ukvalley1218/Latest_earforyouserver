      ...
      $addFields: {
        ...
        userRating: {
          $filter: {
            ...
            as: "review",
            cond: {
              $eq: ["$$review.reviewer", new mongoose.Types.ObjectId(currentUserId)]
...
const getSortOrder = rating => {
  switch (rating) {
    ...
    case 'medium':
      return {
        mediumFirst: -1,
        // Custom field for sorting medium first
        averageRating: -1
      };
    case 'high':
    default:
...
export const filterByReview = async (req, res) => {
  try {
    ...
    let filteredReviews = await Review.find(query).sort(sort).populate({
      path: 'user',
    ...
    if (Address) {
      ...
      filteredReviews = filteredReviews.filter(review => addresses.includes(review.user.companyAddress));
    }
    ...
    if (serviceType) {
      ...
      filteredReviews = filteredReviews.filter(review => serviceTypes.includes(review.user.serviceType));
    }
