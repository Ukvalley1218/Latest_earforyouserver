...
export const createReview = async (req, res) => {
  try {
    ...
    if (existingReview) {
      if (rating) {
        return res.status(400).json({
          message: "You have already rated this user. Only comments/replies are allowed."
        });
      }

      // Add a comment/reply if only a comment is being posted
      if (comment) {
        existingReview.comments.push({
          text: comment,
          commenter: reviewerId
        });
        ...
        return res.status(400).json({
          message: "Comment is required when no rating is provided."
        });
      }
    }
